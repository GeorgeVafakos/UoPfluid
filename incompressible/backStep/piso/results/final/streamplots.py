...
CF =  ax1.contour(x, y, psi, streamlevels100, colors='k',linewidths=0.2)
plt.xticks(np.arange(min(x), max(x)+0.1, 5.0))
plt.yticks(np.arange(min(y), max(y)+0.1, 5.0))
...
plt.title('Re=100', **font_title)
plt.xlabel('x', **font_labels)
plt.ylabel('y', **font_labels)
divider = make_axes_locatable(ax1)
fig.tight_layout()
plt.savefig('CavityStreamRe5000.eps', format='eps', dpi=100, transparent=True, bbox_inches = 'tight', pad_inches = 0) # print graph
