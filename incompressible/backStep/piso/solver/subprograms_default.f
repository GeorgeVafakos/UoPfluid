        ...
        contains
            ...
            subroutine define_vectorBC_type(domain, Var, VarX_BC, VarY_BC, VarZ_BC)
                ...
                do i = 1, domain%NumberBoundaries
                    ...
                    else if (Var%typeBC(i) == 'step_inlet') then
                        VarX_BC(i)%value = merge(0.0, (6.0*VarX_BC(i)%characteristic_valueBC/(y%length-VarX_BC(i)%characteristic_valueBC_2)**2)*(y%nodes(y%in_beg:y%in_end)-VarX_BC(i)%characteristic_valueBC_2)*((y%length-VarX_BC(i)%characteristic_valueBC_2)-(y%nodes(y%in_beg:y%in_end)-VarX_BC(i)%characteristic_valueBC_2)), y%nodes(y%in_beg:y%in_end)<VarX_BC(i)%characteristic_valueBC_2)
                        VarY_BC(i)%value = VarY_BC(i)%characteristic_valueBC
                    else
                    ...
                    do i = 1, domain%NumberBoundaries
                        if (line==Var_str//'_'//boundary_names(i)) then
                            ...
                            if (Var%typeBC(i)=='no_slip') then
                                ...
                                read(unit,*) VarX_BC(i)%characteristic_valueBC
                                read(unit,*) VarX_BC(i)%characteristic_valueBC_2
