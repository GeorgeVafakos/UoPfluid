        ...
        !   Solver:             cavity
        ! 
        !   Discription:        This is an incompressible solver for the solution of the Navier-Stokes equation, using the IPOT algorithm.
        ! 
        ...
        do
            ...
            IPOT_counter = 0
            do
                IPOT_counter = IPOT_counter + 1

                ! Create H matrix
                call create_H()

                ! Calculate pressure
                p%field(nodes_P) = p%field(nodes_P) - r_ipot*nu*div(V)
                ...
                if (abs(V%mean_div_error) <= 1.0d-3 .OR. IPOT_counter==100) then
                    exit
                end if
            end do
            ...
            if (mod(Iter_count,print_to_screen)==0) then
                ...
                print '(A,A29,ES10.2,A16,I9,A16,ES9.2)', 'Pressure', 'Error p =', p%e, 'IPOT Loops =', IPOT_counter, 'Div_Err = ', V%mean_div_error
            ...
            if (V%e(1)<=1.0d-6 .AND. V%e(2)<=1.0d-6 .AND. V%e(3)<=1.0d-6 .AND. p%e<=1.0d-4 .AND. NS_eqn%counter(1)<=1 .AND. NS_eqn%counter(2)<=1 .AND. NS_eqn%counter(3)<=1 .AND. IPOT_counter<=1) then
